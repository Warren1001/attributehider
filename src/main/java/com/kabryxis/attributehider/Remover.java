...
public class Remover implements Listener {
	...
	public Remover(AttributeHider plugin) {
		...
		ProtocolLibrary.getProtocolManager().addPacketListener(new RemoverPacketListener(plugin, this));
		setup();
	...
	private Set<Material> buildSet(String type, Object defaultValue, Supplier<Set<Material>> setIfAll) {
		...
		if (object instanceof Boolean) {
			...
			materials = KMaterial.parseMaterialCollection(lists.getStringList(type), s -> plugin.getLogger().warning(
					String.format("Provided invalid Material for %s: %s", type, s)));
